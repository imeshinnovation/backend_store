...
module.exports = {
  ...
  addarticulo: async (data) =>{
    //try{
        const{id_categoria, codigo, nombre, costo, precio_venta} = data
        const newarticulo = await querys(`INSERT INTO articulo (id_categoria, codigo, nombre, costo, precio_venta) VALUES (${id_categoria}, "${codigo}", "${nombre}", ${costo}, ${precio_venta})`)
